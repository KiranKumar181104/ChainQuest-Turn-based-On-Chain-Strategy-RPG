...
import { 
  ...
  Coins, Gem, Star, Target, Clock, Sparkles, X 
} from 'lucide-react';
...
export const QuickBattle: React.FC<QuickBattleProps> = ({ onComplete, onExit }) => {
  ...
  const generateEnemy = () => {
    ...
    const enemyTypes = [
      ...
      { name: 'Ice Troll', baseAttack: 11, baseDefense: 8, baseHealth: 100 },
      { name: 'Shadow Assassin', baseAttack: 14, baseDefense: 3, baseHealth: 65 },
      { name: 'Stone Golem', baseAttack: 7, baseDefense: 12, baseHealth: 120 }
    ];

    const enemyType = enemyTypes[Math.floor(Math.random() * enemyTypes.length)];
    const levelVariance = Math.max(1, selectedCharacter.level + Math.floor(Math.random() * 3) - 1);
    
    const newEnemy: Enemy = {
      id: `enemy_${Date.now()}`,
      name: enemyType.name,
      level: levelVariance,
      health: enemyType.baseHealth + (levelVariance * 8),
      maxHealth: enemyType.baseHealth + (levelVariance * 8),
      attack: enemyType.baseAttack + (levelVariance * 2),
      defense: enemyType.baseDefense + Math.floor(levelVariance * 1.5),
  ...
  const handleVictory = async () => {
    if (!selectedCharacter || !profile || !enemy) return;

    try {
      const baseReward = 50 + (enemy.level * 10);
      const expGained = baseReward;
      const goldGained = baseReward * 2;
      const gemsGained = Math.floor(enemy.level / 3);
      ...
      await supabase
        ...
        .insert([{
          ...
          player1_character_id: selectedCharacter.id,
          player2_character_id: null,
          winner_id: profile.user_id,
          battle_log: battleLog,
          rewards: { gold: goldGained, gems: gemsGained, experience: expGained },
      ...
      <div className="relative z-10 p-4">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-center">
            <h1 className="text-2xl font-bold text-white">Quick Battle</h1>
            <p className="text-slate-300">Fight for glory and rewards!</p>
          </div>
            ...
            >
              {battlePhase === 'victory' ? (
                <>
                  ...
                  <p className="text-slate-300 mb-6">You have defeated the {enemy.name}!</p>
